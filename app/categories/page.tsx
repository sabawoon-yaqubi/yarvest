...
export default function CategoriesPage() {
  ...
  const t = useSafeTranslations("categories")
            ...
            {categories.length > 0 && (
              <div className="mt-6 inline-flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur-sm rounded-full shadow-sm border border-gray-200/50">
                ...
                <span className="text-sm font-medium text-gray-700">
                  {t("categoriesAvailable", { count: categories.length })}
                </span>
              </div>
            )}
          </div>
          ...
          {searchQuery && categories.length > 0 && (
            <div className="mb-6 text-center">
              <p className="text-sm sm:text-base text-gray-600 inline-flex items-center gap-2 px-4 py-2 bg-white/60 backdrop-blur-sm rounded-full border border-gray-200/50">
                {t("showing")} <span className="font-bold text-[#5a9c3a]">{filteredCategories.length}</span> {t("of")} <span className="font-semibold">{categories.length}</span> {t("categories")}
              </p>
            </div>
          )}
          ...
          {categoriesLoading ? (
                ...
                <div className="px-6 py-16 sm:py-20 bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg border border-gray-200/50">
                  <div className="max-w-2xl mx-auto text-center">
                    ...
                    <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">{t("errorLoading")}</h1>
                ...
                <div className="text-center py-16 sm:py-24">
                  ...
                  <h3 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3">{t("noCategoriesFound")}</h3>
                          ...
                          {category.products_count !== undefined && category.products_count > 0 && (
                            <div className="absolute top-4 right-4 z-10">
                              <Badge className="bg-white/95 backdrop-blur-sm text-gray-900 font-semibold px-3 py-1.5 shadow-lg border-0 rounded-full">
                                {category.products_count} {category.products_count === 1 ? t("item") : t("items")}
                              </Badge>
                            </div>
                          )}
                              ...
                              {category.products_count !== undefined && category.products_count > 0 ? (
                                <>
                                  ...
                                  <span className="font-medium">
                                    {category.products_count} {category.products_count === 1 ? t("item") : t("items")} {t("available")}
                                  </span>
                                </>
                              ) : (
                                <>
                                  <div className="w-2 h-2 rounded-full bg-white/80 animate-pulse" />
                                  <span className="font-medium">{t("exploreCollection")}</span>
                                </>
                              )}
                            </div>
                          </div>
