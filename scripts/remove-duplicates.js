...
console.log("🔍 Removing duplicate farms from CSV...\n");
...
// Remove duplicates based on name and full_address (same logic as scraping script)
const uniqueFarms = [];
const seen = new Set();
...
for (const farm of farms) {
  ...
  const nameKey = farm.name.toLowerCase().trim();
  const addressKey = (farm.full_address || farm.address || "").toLowerCase().trim();
  
  // Also try city + state if full_address is empty
  const locationKey = addressKey || 
    `${farm.city || ""}, ${farm.state || ""}`.toLowerCase().trim();
